 ...
 * 
 */
public class ManageChangePersonalConfirmAction extends ChangeBaseAction {
	...
	public String getActionContext() {
		return "/change/managechangepersonalconfirm.action";
	}

	@Override
	public String executeFunction() throws Exception {
		String user_id = getCurUser().getUser_iidd();
		Map<String, Object> map = new HashMap<String, Object>();
		// 判断载体归属转换模式
		int mode = basicService.getSysConfigItemValue(SysConfigItem.KEY_DEPTTOPERSON_MODE).getStartuse();
		if (mode == 0) {// 个人确认模块开启
			map.put("change_type", "toPERSON");
			map.put("user_iidd", user_id);
		} else {
			map.put("change_type", "none");
		}
