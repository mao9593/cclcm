package hdsec.web.project.education.upload;
 ...
 * 
 */
public class UploadLearningFile extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String tempPath = "";
		try {
			tempPath = BMPropertyUtil.getLearningFileStorePath();
		} catch (Exception e1) {
			logger.error(e1.getMessage());
			e1.printStackTrace();
		}
		if (!StringUtils.hasLength(tempPath.trim())) {
			logger.error("上传保密学习文件存放路径未设置");
		}
		...
		try {
			...
			if (path.canWrite()) {
				...
				logger.info("上传安装文件：" + tempPath + "\\" + uploadFileName);
			} else {
				logger.error("指定的安装文件存放路径[" + tempPath + "]没有可写权限，请设置该路径权限");
			}
		} catch (Exception e) {
