 ...
 * 
 */
public class ExportDestroyCDAction extends AutoCommonExportAction {
	...
	private final String filename = "已销毁光盘台账记录-" + sdf1.format(new Date()) + ".xls";
	private final String sheetName = "已销毁光盘台账记录";
	private String cols= "";
	...
	public String executeFunction() throws Exception {
		cols=basicService.getSysConfigItemValue("exportdestroycd").getItem_value();
		...
		map.put("cd_state", 4);// 类型为已销毁
		map.put("admin_dept_ids", dept_ids.split(","));
		map.put("destroy_time_sortable", true);
