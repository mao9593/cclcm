...
public class ExportPaperStaticBySeclvAction extends LogExportXlsAction {
	...
	private final String filename = "纸张密级统计-" + sdf1.format(new Date()) + ".xls";
	private final String sheetName = "纸张统计";
	private final String[] titles = { "序号", "部门名称", "密级", "页数", "合计", };
	...
	public String executeFunction() throws Exception {
		...
		paperStaticList = basicService.getPaperStaticListBySeclv(map);
	...
	public void insertIntoOneSheet(int pageIndex, int from, int to, String sheetName, String[] titles)
		...
		// 合并单元格
		int total_size = paperStaticList.size();
		int seclvNum = userService.getSecLevel().size();
		int deptNum = total_size / seclvNum;
		for (int i = 0; i < deptNum; i++) {
			sh.addMergedRegion(new CellRangeAddress(seclvNum * i + 2, seclvNum * (i + 1) + 1, 1, 1));
			sh.addMergedRegion(new CellRangeAddress(seclvNum * i + 2, seclvNum * (i + 1) + 1, 4, 4));
		}
		// 插入最后一行（总计）
	...
	private void insertRow(HSSFSheet sh, PaperStatic paper_static, int rownum) {
		...
		cteateCell(row, index++, changeNullOrEmpty(paper_static.getSeclv_name()));
