...
public class ExportPaperStaticByDoubleAction extends LogExportXlsAction {
	...
	private final String filename = "纸张单双面统计-" + sdf1.format(new Date()) + ".xls";
	private final String sheetName = "纸张统计";
	private final String[] titles = { "序号", "部门名称", "单双面", "页数", "合计", };
	...
	public String executeFunction() throws Exception {
		...
		paperStaticList = basicService.getPaperStaticListByDouble(map);
		// 合计
	...
	public void insertIntoOneSheet(int pageIndex, int from, int to, String sheetName, String[] titles)
		...
		int total_size = paperStaticList.size();
		int doubleNum = 2;
		int deptNum = total_size / doubleNum;
		for (int i = 0; i < deptNum; i++) {
			sh.addMergedRegion(new CellRangeAddress(doubleNum * i + 2, doubleNum * (i + 1) + 1, 1, 1));
			sh.addMergedRegion(new CellRangeAddress(doubleNum * i + 2, doubleNum * (i + 1) + 1, 4, 4));
		}
		// 插入最后一行（总计）
