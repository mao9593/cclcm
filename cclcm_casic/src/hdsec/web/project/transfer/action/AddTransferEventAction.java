...
public class AddTransferEventAction extends TransferBaseAction {
	...
	private EventTransfer getTransferEntityByID() {
		EntityCD entity = transferService.getOneCDLedgerById(id);
		...
		if (null != entity) {
			...
			transfer.setUsage_name(entity.getUser_name());
			...
			transfer.setProject_name(entity.getProject_code());
			transfer.setApply_time(new Date());
			transfer.setAccept_user_iidd(accept_user_iidd);
			transfer.setAccept_dept_id(entity.getDept_id());
		}
		return transfer;
	}
