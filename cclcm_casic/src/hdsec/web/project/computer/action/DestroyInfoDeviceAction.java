...
public class DestroyInfoDeviceAction extends ComputerBaseAction {
	...
	private String jobType = JobTypeEnum.DEVICE_DES.getJobTypeCode();
	...
	public String executeFunction() throws Exception {
		...
		if (StringUtils.hasLength(event_code)) {
			...
			device.setEvent_type(3);
			device.setJobType(JobTypeEnum.valueOf(jobType));
			computerService.addInfoDeviceEvent(device, next_approver);
			insertCommonLog("添加信息设备报废申请[" + event_code + "]");
			return "ok";
		} else {
			event_code = getCurUser().getUser_iidd() + "_DEVICEDESTROY_" + System.currentTimeMillis();
			return SUCCESS;
		}
	}
}