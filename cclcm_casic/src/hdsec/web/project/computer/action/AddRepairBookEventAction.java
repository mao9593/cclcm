 ...
 * 
 */
public class AddRepairBookEventAction extends ComputerBaseAction {
	...
	private String jobType = JobTypeEnum.BOOK_REPAIR.getJobTypeCode();
	...
	public String executeFunction() throws Exception {
		if (StringUtils.hasLength(event_code)) {
			...
			ChangeDeviceEvent event = new ChangeDeviceEvent(event_code, user_iidd, dept_id, seclv_code, usage_code,
					project_code, summ, user_phone, book_barcode, 12);// 笔记本维修申请表

			event.setJobType(JobTypeEnum.valueOf(jobType));
			computerService.addComputerEvent(event, next_approver);
			insertCommonLog("添加笔记本维修申请[" + event_code + "]");
			return "ok";
		} else {
			event_code = getCurUser().getUser_iidd() + "_REPAIRBOOK_" + System.currentTimeMillis();
