...
public class ApproveComputerJobAction extends ComputerBaseAction {
	...
	protected void getComputerInfoSpecial(ChangeDeviceEvent event1) {
		...
		if (event1.getEvent_type().equals(1)) {// 新增计算机网络机
			...
			// event1 = null;
			// return "ok_internet";
		} else if (event1.getEvent_type().equals(2)) {// 新增计算机单机
		...
		} else if (event1.getEvent_type().equals(3)) {// 计算机变更
			...
			if (seclv_code == null) {
				EntityComputer computer_temp = new EntityComputer(event_device_content.getComputer_barcode(), "", null,
						...
						event_device_content.getComputer_code(), 0, event_device_content.getHdisk_no(), "", null,
				...
				EntityComputer computer_temp = new EntityComputer(event_device_content.getComputer_barcode(), "",
						...
						duty_dept_name, event_device_content.getDuty_entp_id(),
						event_device_content.getComputer_code(), 0, event_device_content.getHdisk_no(), "", null,
						event_device_content.getComputer_ip(), event_device_content.getComputer_mac(),
						event_device_content.getStorage_area(), event_device_content.getStorage_location(),
						event_device_content.getSumm(), "", "", null, null, event_device_content.getOldconf_code(),
			...
			// return "ok_change";
		} else if (event1.getEvent_type().equals(4)) {// 计算机报废
			Map<String, Object> map = new HashMap<String, Object>();
	...
	public String executeFunction() throws Exception {
		if (history.equals("Y")) {
			...
			if (StringUtils.hasLength(approved)) {// 审批结果
				...
				event = computerService.getChangeDeviceEventByJobCode(job_code);
				if (event.getEvent_type().equals(1)) {// 新增网络版计算机
					if (listSize == 5) {
						...
						other_opinion = other_opinion + "Key号:  " + key_code + "<br>" + "计算机IP:  " + computer_ip
								+ "<br>" + "交换机IP:  " + switch_num + "<br>" + "交换机端口:  " + switch_point + "<br>"
								+ "掩码:  " + mark_code + "<br>" + "硬盘序列号:  " + hdisk_no + "<br>" + "MAC地址:  "
								+ computer_mac + "<br>" + "操作系统版本:  " + computer_os + "<br>" + "操作系统安装时间:  " + time1
								+ "<br>" + "已安装软件:  " + softName;
				...
				} else if (event.getEvent_type().equals(3)) {// 计算机变更
					if (listSize == 4) {
						String other_opinion = "";
				...
				} else if (event.getEvent_type().equals(4)) {// 计算机报废
					if (listSize == 3) {
						String other_opinion = "";
						if (isrecycle.equals("是")) {
							Map<String, Object> map1 = new HashMap<String, Object>();
