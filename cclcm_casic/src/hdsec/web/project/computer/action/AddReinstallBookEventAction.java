 ...
 * 
 */
public class AddReinstallBookEventAction extends ComputerBaseAction {
	...
	private String jobType = JobTypeEnum.BOOK_REINSTALL.getJobTypeCode();
	...
	public String executeFunction() throws Exception {
		if (StringUtils.hasLength(event_code)) {
			...
			ChangeDeviceEvent event = new ChangeDeviceEvent(event_code, user_iidd, dept_id, seclv_code, usage_code,
					project_code, summ, user_phone, book_barcode, 14);// 笔记本重装系统申请表

			event.setJobType(JobTypeEnum.valueOf(jobType));
			computerService.addComputerEvent(event, next_approver);
			insertCommonLog("添加笔记本重装系统申请[" + event_code + "]");
			return "ok";
		} else {
			event_code = getCurUser().getUser_iidd() + "_REINSTALLBOOK_" + System.currentTimeMillis();
