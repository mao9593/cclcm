 ...
 * 
 */
public class ApproveSpecialPrintJobAction extends PrintBaseAction {
	...
	public String executeFunction() throws Exception {
		if (StringUtils.hasLength(approved)) {// 审批结果
			...
			String event_names = "";
			List<OaPrintEvent> events = printService.getSpecialPrintEventListByJobCode(job_code);
			for (OaPrintEvent event : events) {
				event_names += event.getPaper_name() + ",";
			}
			insertCommonLog("审批特殊打印任务，文件列表[" + event_names + "]");
			return "ok";
		} else {
			...
			eventList = printService.getSpecialPrintEventListByJobCode(job_code);
