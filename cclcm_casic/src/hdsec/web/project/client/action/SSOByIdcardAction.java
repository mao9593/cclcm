 ...
 * 
 */
public class SSOByIdcardAction extends ActionSupport {
	...
	public String execute() {
		try {
			if (isEncrypt.equals("isEn")) {// 加密验证
				if (StringUtils.hasLength(verifySSO)) {
					String decryptedSSO = new String(new sun.misc.BASE64Decoder().decodeBuffer(verifySSO));
					logger.info("decryptedSSO :" + decryptedSSO);
					if (decryptedSSO.indexOf("PID=") == -1) {
						logger.warn("decryptedSSO not including 'PID=', replace space with '+' in verifySSO");
						verifySSO = verifySSO.replace(' ', '+');
						decryptedSSO = new String(new sun.misc.BASE64Decoder().decodeBuffer(verifySSO));
						logger.info("once more decryptedSSO :" + decryptedSSO);
					}
					if (decryptedSSO.indexOf("PID=") == -1) {
						throw new Exception("信息验证失败，无法登录");
					}
					if (checkSSOInfo(decryptedSSO)) {
						String tmp[] = decryptedSSO.split(",");
						PID = tmp[1].substring("PID=".length());
					} else {
						// throw new Exception("信息验证失败，无法登录");
						String tmp[] = decryptedSSO.split(",");
						PID = tmp[1].substring("PID=".length());
	...
	private boolean checkSSOInfo(String decryptedSSO) {
		boolean ret = decryptedSSO.equals("userID=" + userID + ",PID=" + PID + ",");
		logger.info("checkSSOInfo:" + ret);
		return ret;
	}
	...
	private boolean checkUserInfo() throws Exception {
		...
		String projectDetails = "<?xml version=\"1.0\" encoding=\"GB2312\"?><root><data><sessionID>" + sessionID
				+ "</sessionID><msg></msg></data></root>";
		/* 调用门户系统WebService服务，返回结果 */
		String[] param = { "common", "0", "biz.bizCheckSessionID", projectDetails };
