 ...
 * 
 */
public class ApproveCopyJobAction extends CopyBaseAction {
	...
	public String executeFunction() throws Exception {
		if (StringUtils.hasLength(approved)) {// 审批结果
			...
			if (jobdetail.getNext_approver().contains(getCurUser().getUser_iidd())) {
				...
				for (SysProxy sysProxy : approvers) {
					if (jobdetail.getNext_approver().contains(sysProxy.getUser_iidd())) {
						user= new ApproverUser(sysProxy.getUser_iidd(), userService.getUserNameByUserId(sysProxy.getUser_iidd()));
			...
			insertCommonLog("审批复印任务，审批单编号[" + job_code + "]");
			return "ok";
		} else {
			...
			try {
				...
				for (ApproverUser user : userList) {
					List<SecLevel> seclvList = userService.getCopySecLevelByUser(user.getUser_iidd());
