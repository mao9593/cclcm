...
import hdsec.web.project.asset.service.AssetService;
import hdsec.web.project.asset.action.AssetBaseAction;
 ...
 * 
 */
public class PropertyExportAction extends LogExportXlsAction {
	...
	private final String SELFPROPERTY = "selfproperty";
	...
	public void exportFile(OutputStream os, Map<String, Object> map,
			String sheetName, String[] titles, String type) throws Exception {
		...
		} else if (SELFPROPERTY.equals(type)) {
			total_size = assetService.getSelfPropertyLedgerSize(map);
		...
		for (int i = 0; i < count; i++) {
			...
			if (ENTITY_PAPER.equals(type)) {
				...
				ledgers = borrowService
						.getPersonPaperBorrowLedgerList(map, rbs);
			} else if (STORAGE.equals(type)) {
				ledgers = storageService.getStorageList(map);
			} else if (SELFPROPERTY.equals(type)) {
				ledgers = assetService.getPropertyList(map);
			} else if (CVS.equals(type)) {
				ledgers = clientService.getCVSListByCondition(map);
			}
			// long end_time = new Date().getTime();
			// System.out.println("total time : " + (end_time - start_time) +
			// "  total size:" + ledgers.size());
			if (count - 1 != i) {
				insertIntoOneSheet(i, i * CCLCMConstants.SHEET_SIZE - begin,
						(i + 1) * CCLCMConstants.SHEET_SIZE - 1 - begin,
						sheetName, titles, type);
			} else {
				insertIntoOneSheet(i, i * CCLCMConstants.SHEET_SIZE - begin,
						total_size - 1 - begin, sheetName, titles, type);
			}
		}
	...
	public void insertIntoOneSheet(int pageIndex, int from, int to,
			String sheetName, String[] titles, String type) throws IOException {
				...
				if (ENTITY_PAPER.equals(type)) {
					...
					if (entity != null
						...
						if (entity.getCreate_type().equals("COPY")) {
							event = copyService.getCopyEventByCopyCode(entity
									.getEvent_code());
						}
						...
						if (entity.getCreate_type().equals("LEADIN")) {
							event = enterService
									.getEnterEventByEnterCode(entity
											.getEvent_code());
						}
						if (entity.getCreate_type().equals("PRINT")) {
							event = printService
									.getPrintEventByPrintCode(entity
											.getEvent_code());
						}
						if (event != null) {
							...
							if (job != null) {
								...
								recordList = activitiService
										.getProcessRecordList(record);
								for (ProcessRecord processRecord : recordList) {
									approveInfo += processRecord.getUser_name()
											+ ","
											+ processRecord.getDept_name()
											+ ","
											+ sdf.format(processRecord
													.getOp_time()) + ","
											+ processRecord.getOpinion()
											+ ",\n";
								}
								approveInfo = approveInfo.substring(0,
										approveInfo.length() - 1);
							}
					...
					if (StringUtils.hasLength(entity.getJob_code())) {
						...
						if (job1 != null) {
							...
							recordList1 = activitiService
									.getProcessRecordList(record1);
							for (ProcessRecord processRecord1 : recordList1) {
								cycleApproveInfo += processRecord1
										.getUser_name()
										+ ","
										+ processRecord1.getDept_name()
										+ ","
										+ sdf.format(processRecord1
												.getOp_time())
										+ ","
										+ processRecord1.getOpinion() + ",\n";
							}
							cycleApproveInfo = cycleApproveInfo.substring(0,
									cycleApproveInfo.length() - 1);
						}
					}
					insertPaperRow(sh, entity, usage_name, approveInfo,
							cycleApproveInfo, i - from + 1);
				} else if (ENTITY_CD.equals(type)) {
					...
					if (entity != null
						...
						if (entity.getCreate_type().equals("BURN")) {
							event = burnService.getBurnEventByBurnCode(entity
									.getEvent_code());
						}
						if (entity.getCreate_type().equals("LEADIN")) {
							event = enterService
					...
					insertCDRow(sh, entity, usage_name, approveInfo,
							cycleApproveInfo, i - from + 1);
				} else if (ENTITY_DEVICE.equals(type)) {
					...
					EntityProperty entity = (EntityProperty) ledgers.get(i);
					insertPropertyRow(sh, entity, i - from + 1);
				} else if (CVS.equals(type)) {
	...
	private void insertPropertyRow(HSSFSheet sh, EntityProperty entity,
			int rownum) {
		...
		cteateCell(row, index++, changeNullOrEmpty(entity.getProperty_kind()));
		cteateCell(row, index++, changeNullOrEmpty(entity.getProperty_name()));
		cteateCell(row, index++,
				changeNullOrEmpty(entity.getProperty_barcode()));
		cteateCell(row, index++,
				changeNullOrEmpty(entity.getProperty_status_str()));
		cteateCell(row, index++, changeNullOrEmpty(entity.getIdentity_code()));
		cteateCell(row, index++, changeNullOrEmpty(entity.getProperty_no()));
		cteateCell(row, index++, changeNullOrEmpty(entity.getVoucher_no()));
		cteateCell(row, index++, changeNullOrEmpty(entity.getFactory_no()));
		cteateCell(row, index++, changeNullOrEmpty(entity.getFactory_date()));
		cteateCell(row, index++, changeNullOrEmpty(entity.getUse_date()));
		cteateCell(row, index++, changeNullOrEmpty(entity.getUnit_price()));
		cteateCell(row, index++,
				changeNullOrEmpty(entity.getProperty_standard()));
		cteateCell(row, index++, changeNullOrEmpty(entity.getProperty_type()));
		cteateCell(row, index++, changeNullOrEmpty(entity.getSupplier()));
		cteateCell(row, index++, changeNullOrEmpty(entity.getOriginal_value()));
		cteateCell(row, index++, changeNullOrEmpty(entity.getAverage_equity()));
		cteateCell(row, index++, changeNullOrEmpty(entity.getSetup_place()));
		cteateCell(row, index++, changeNullOrEmpty(entity.getIn_time()));
		cteateCell(row, index++, changeNullOrEmpty(entity.getWaste_time()));
	...
	private void insertStorageRow(HSSFSheet sh, EntityStorage entity, int rownum) {
		...
		cteateCell(row, index++,
				changeNullOrEmpty(entity.getStorage_status_name()));
	...
	private void insertDeviceRow(HSSFSheet sh, EntityDevice entity, int rownum) {
		...
		cteateCell(row, index++,
				changeNullOrEmpty(entity.getBorrow_user_name()));// 借用人
		cteateCell(row, index++,
				changeNullOrEmpty(entity.getBorrow_dept_name()));// 借用人部门
		...
		cteateCell(row, index++,
				changeNullOrEmpty(entity.getDevice_status_name()));// 状态
	...
	private void insertCDRow(HSSFSheet sh, EntityCD cd, String usage_name,
			String approveInfo, String cycleApproveInfo, int rownum) {
		int index = 0;
		HSSFRow row = sh.createRow(rownum);
		...
		cteateCell(row, index++,
				(cd.getFile_num() == null ? 0 : cd.getFile_num()) + "");// 文件数量
	...
	private void insertPaperRow(HSSFSheet sh, EntityPaper paper,
			String usage_name, String approveInfo, String cycleApproveInfo,
			int rownum) {
		int index = 0;
		HSSFRow row = sh.createRow(rownum);
		cteateCell(row, index++, rownum + "");// 序号
		...
		cteateCell(row, index++,
				changeNullOrEmpty(paper.getPrint_result_name()));// 打印结果
		...
		cteateCell(row, index++,
				(paper.getPage_count() == null ? 0 : paper.getPage_count())
						+ "");// 页数
		...
		cteateCell(row, index++,
				changeNullOrEmpty(paper.getPrint_direct_name()));// 横纵向
		cteateCell(row, index++,
				changeNullOrEmpty(paper.getPrint_double_name()));// 单双面
		...
		cteateCell(row, index++,
				changeNullOrEmpty(paper.getRetrieve_type_name()));// 回收方式
		cteateCell(row, index++,
				changeNullOrEmpty(paper.getRetrieve_user_name()));// 闭环处理人(回收、外发、归档)
		cteateCell(row, index++, changeNullOrEmpty(cycleApproveInfo));// 闭环审批信息
		cteateCell(row, index++,
				changeNullOrEmpty(paper.getRetrieve_box_code()));// 回收柜标识
		cteateCell(row, index++, changeNullOrEmpty(paper.getDestroy_time()));// 销毁时间
		cteateCell(row, index++,
				changeNullOrEmpty(paper.getDestroy_user_name()));// 销毁人
		...
		cteateCell(row, index++,
				changeNullOrEmpty(paper.getExpire_status_name()));// 到期状态
		cteateCell(row, index++, changeNullOrEmpty(usage_name));// 用途
		// cteateCell(row, index++, changeNullOrEmpty(paper.getUser_name()));//
		// 制作人
		// cteateCell(row, index++, changeNullOrEmpty(paper.getDept_name()));//
		// 制作人部门
