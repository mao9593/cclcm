 ...
 * 
 */
public class ExportUserLoginLogAction extends LogExportXlsAction {
	...
	private String filename = "用户登录日志-" + sdf1.format(new Date()) + ".xls";
	private String sheetName = "用户登录日志";
	private HSSFWorkbook wb = new HSSFWorkbook();
	private String[] titles = { "用户ID", "用户名", "归属部门", "日志内容", "操作结果", "时间", "登录IP", "主机名称" };
	...
	public String executeFunction() throws Exception {
		...
		while (prestart || (logList != null && size == SHEET_ROW_NUM)) {
			...
			logList = logService.getUserLoginLog(map, new RowBounds(beginIndex, SHEET_ROW_NUM));
			size = logList.size();
	...
	private void insertRow(HSSFRow row, UserLoginLog log) {
		int index = 0;
