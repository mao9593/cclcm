 ...
 * 电子文件导入审批任务
 * 
 * @author guoxh 2016-9-30 16:43:54
 */
public class ApproveInputJobAction extends InputBaseAction {
	...
	public String executeFunction() throws Exception {
		if (StringUtils.hasLength(approved)) {// 审批结果
			...
			// 若审批已驳回,则删除提交的电子文件
			String job_status = basicService.getProcessJobByCode(job_code).getJob_status();
			if (job_status.equals("false")) {
				String event_code = inputService.getEventCodeByJobCode(job_code);
			...
			insertCommonLog("审批输入信息，审批单编号[" + job_code + "]");
			return "ok";
		} else {
