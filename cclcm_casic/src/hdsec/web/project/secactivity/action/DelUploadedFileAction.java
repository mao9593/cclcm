 ...
 * 
 */
public class DelUploadedFileAction extends SecuActiBaseAction {
	...
	public String executeFunction() {
		if (file_type.equalsIgnoreCase("temp")) {
			...
			String tempPath = "";
			try {
				tempPath = BMPropertyUtil.getSecActivityStrorePath();
			} catch (Exception e) {
			...
			String storePath = "";
			try {
				storePath = BMPropertyUtil.getSecActivityStrorePath();
			} catch (Exception e) {
				logger.error(e.getMessage());
				e.printStackTrace();
			}
			...
			if (path.isDirectory()) {
				...
				if (files.length == 0) {
					...
					if (files[0].delete()) {
						setChkResult("删除成功");
						UserSecActiEvent event = secactivityservice.getSecActiEvent(event_code);
