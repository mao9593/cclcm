...
public class ApprovePunishJobAction extends SecManageBaseAction {
	...
	private int getPunishEectifyApp(PunishRectifyEvent event) {
		if (listSize == 1) {
			userList = getFixApprover(event.getPunish_dept_id(), "10");
		} else if (listSize == 0) {
			userList = getFixApprover(event.getPunish_dept_id(), "30");
		}
		if (userList == null) {
			return 1;
		}
	...
	public String executeFunction() throws Exception {
		if (history.equals("Y")) {
			...
			eventList = secManageService.getPunishEventListByJobCode(job_code);
			...
			if (StringUtils.hasLength(approved)) {// 审批结果
				...
				insertCommonLog("违规整改，审批单编号[" + job_code + "]");
				return "ok";
			} else {
				job = basicService.getProcessJobByCode(job_code);
				ProcessRecord record = new ProcessRecord();
				...
				try {
					...
					// 部门不合规事项整改督查 模块 审批人特殊选择 --gj
					if (event.getApply_type() == 2) {
						int result = getPunishEectifyApp(event);
						if (result == 1) {
							throw new Exception("WARN:部门[" + event.getPunish_dept_name()
									+ "]内不存在拥有角色[可审批用户]或[保密管理员]的用户，没有符合条件的审批人，请联系管理员。");
						}
					} else if (event.getApply_type() == 1) {
						if (listSize == 0) {
							userList = getFixApprover(event.getPunish_dept_id(), "10");
							if (userList == null) {
								throw new Exception("WARN:部门[" + event.getPunish_dept_name()
										+ "]内不存在拥有角色[可审批用户]的用户，没有符合条件的审批人，请联系管理员。");
							}
						}
					}
