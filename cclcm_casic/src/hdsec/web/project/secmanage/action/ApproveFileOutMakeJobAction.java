 ...
 */
public class ApproveFileOutMakeJobAction extends SecManageBaseAction {
	...
	public void setEvent(FileOutMakeEvent event) {
		this.event = event;
	}
	...
	public String executeFunction() throws Exception {
		if (history.equals("Y")) {
			...
			eventList = secManageService.getFileOutMakeEventListByJobCode(job_code);
			...
			if (StringUtils.hasLength(approved)) {// 审批结果
				...
				insertCommonLog("审批涉密文件资料外出制作任务，审批单编号[" + job_code + "]");
				return "ok";
			} else {
				job = basicService.getProcessJobByCode(job_code);
				ProcessRecord record = new ProcessRecord();
