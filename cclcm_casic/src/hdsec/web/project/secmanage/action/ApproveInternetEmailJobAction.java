 ...
 * @author guojiao
 */
public class ApproveInternetEmailJobAction extends SecManageBaseAction {
	...
	public void setEvent(InternetEmailEvent event) {
		this.event = event;
	}
	...
	public String executeFunction() throws Exception {
		if (history.equals("Y")) {
			...
			eventList = secManageService.getInternetEmailListByJobCode(job_code);
			...
			if (StringUtils.hasLength(approved)) {// 审批结果
				...
				insertCommonLog("审批外网电子邮件任务，审批单编号[" + job_code + "]");
				return "ok";
			} else {
				job = basicService.getProcessJobByCode(job_code);
				ProcessRecord record = new ProcessRecord();
