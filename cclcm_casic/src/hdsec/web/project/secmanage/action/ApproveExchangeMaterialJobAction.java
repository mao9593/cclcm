 ...
 * @author guojiao
 */
public class ApproveExchangeMaterialJobAction extends SecManageBaseAction {
	...
	private void getMoreInfo(ExchangeMaterialEvent event1, List<MaterialInfo> material1) {
		...
		if (!event1.getFile_list().equals("")) {
			String[] temp = event1.getFile_list().split(BMSysConfigItem.COMMON_SEPARATOR_END);
			for (int i = 0; i < temp.length; i++) {
				String[] temp_info = temp[i].split("\\|");
				material1.add(new MaterialInfo(temp_info[1], temp_info[2]));
			}
		}
	}
	...
	public String executeFunction() throws Exception {
		if (history.equals("Y")) {
			...
			eventList = secManageService.getExchangeMaterialListByJobCode(job_code);
			event = eventList.get(0);
			material = new ArrayList<MaterialInfo>();
			getMoreInfo(event, material);
			return SUCCESS;
		} else {
