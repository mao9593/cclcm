...
public class ApproveExhibitionJobAction extends SecManageBaseAction {
	...
	private ExhibitionEvent event = null;
	...
	public ExhibitionEvent getEvent() {
		return event;
	}
	...
	public String executeFunction() throws Exception {
		if (history.equals("Y")) {
			if (job_code.length() != 0) {
				...
				job_code = secManageService.getExhibitionJobCodeByEventCode(event_code);
				job = basicService.getProcessJobByCode(job_code);
			}
			...
			eventList = secManageService.getExhibitionListByJobCode(job_code);
			// add by liuyaling
			...
			if (StringUtils.hasLength(approved)) {// 审批结果
				...
				insertCommonLog("审批展示展览保密审查任务，审批单编号[" + job_code + "]");
				return "ok";
			} else {
				job = basicService.getProcessJobByCode(job_code);
				ProcessRecord record = new ProcessRecord();
