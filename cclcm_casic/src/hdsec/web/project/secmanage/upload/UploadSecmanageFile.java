package hdsec.web.project.secmanage.upload;
 ...
 * 
 */
public class UploadSecmanageFile extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (!event_code.equals("") && event_code.contains("USERINFO")) {
			type = 1;
		}
		try {
			if (type == 1) {
				tempPath = BMPropertyUtil.getSecManageStrorePath() + "/" + "headshot";
			} else
		...
		if (!StringUtils.hasLength(tempPath.trim())) {
			logger.error("上传保密管理文件临时存放路径未设置");
		}
		...
		try {
			...
			if (path.exists() && type == 1) {
				String[] str = path.list();
				for (int a = 0; a < str.length; a++) {
					File dname = new File(tempPath + "/" + str[a]);
					dname.delete();
				}
			}
			if (!path.exists()) {
			...
			if (path.canWrite()) {
				...
				logger.error("指定的上传保密管理文件临时存放路径[" + tempPath + "]没有可写权限，请设置该路径权限");
			}
		} catch (Exception e) {
