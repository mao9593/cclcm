 ...
 * 
 */
public class DelUploadedFileAction extends SecplaceBaseAction {
	...
	public String executeFunction() {
		if (file_type.equalsIgnoreCase("temp")) {
			...
			String tempPath = "";
			try {
				tempPath = BMPropertyUtil.getSecplaceFileStorePath();
			} catch (Exception e) {
			...
			String storePath = "";
			try {
				storePath = BMPropertyUtil.getSecplaceFileStorePath();
			} catch (Exception e) {
				logger.error(e.getMessage());
				e.printStackTrace();
			}
			...
			if (path.isDirectory()) {
				...
				if (files.length == 0) {
					...
					if (files[0].delete()) {
						setChkResult("删除成功");
						EnterSecplaceEvent event = secplaceService.getEnterSecplaceEventByEventCode(event_code);
						...
						secplaceService.updateEnterSecplaceEventFileInfo(event_code, fileNum, newFileList);
					} else {
						setChkResult("删除文件[" + file_name + "]失败");
					}
				}
