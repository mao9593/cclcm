 ...
 * 
 */
public class AddBurnerAction extends BasicBaseAction {
	...
	public String executeFunction() throws Exception {
		if (burner_name.isEmpty()) {
			return SUCCESS;
		} else {
			String burner_code = "BNR-" + String.valueOf(UUID.randomUUID().toString());
			SysBurner burner = new SysBurner(burner_code, burner_name, burner_path, burner_type, burner_brand,
					burner_model, burner_location, dept_id, new Date(), console_code, seclv_code, burner_usefor,
					mfp_code);
			basicService.addSysBurner(burner);
			insertAdminLog("添加一体机[" + burner_name + "]");
			return "ok";
		}
	}
}
