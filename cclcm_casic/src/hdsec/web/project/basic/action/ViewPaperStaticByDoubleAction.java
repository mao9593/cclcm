 ...
 * 
 */
public class ViewPaperStaticByDoubleAction extends BasicBaseAction {
	...
	public String executeFunction() throws Exception {
		...
		if (query.equalsIgnoreCase("Y")) {
			...
			int colorNum = 2;
			for (int i = 0; i < paperStaticList.size() / colorNum; i++) {
				int dept_pages = 0;
				for (int j = 0; j < colorNum; j++) {
					dept_pages += paperStaticList.get(colorNum * i + j).getPage_count();
				}
