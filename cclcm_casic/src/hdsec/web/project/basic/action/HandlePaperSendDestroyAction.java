 ...
 * 文件台帐处理申请
 * 
 * @author gaoxm 2015-3-11
 */
public class HandlePaperSendDestroyAction extends BasicBaseAction {
	...
	public String executeFunction() throws Exception {
		if (addjob.equalsIgnoreCase("Y")) {
			try {
				basicService.handlePaperSendDestroy(getCurUser().getUser_iidd(), getCurUser().getDept_id(), seclv_code,
						jobType, next_approver, StringUtil.stringArrayToList(_chk.split(",")));
				insertCommonLog("提交文件载体" + jobType.getJobTypeName() + "审批申请：条码号[" + _chk + "].");
			} catch (Exception e) {
