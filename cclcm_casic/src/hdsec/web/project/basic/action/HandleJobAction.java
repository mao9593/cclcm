 ...
 * 
 */
public class HandleJobAction extends BasicBaseAction {
	...
	public String executeFunction() throws Exception {
		if (StringUtils.hasLength(approved)) {// 审批结果
			...
			basicService.approveJob(job_code, user, approver, approved, opinion, "_" + type);
			insertCommonLog("审批流程申请任务[" + job_code + "]");
			return "ok";
		} else {
			job = basicService.getProcessJobByCode(job_code);
			// 获取监销人名字
			if ((job_code.contains("DESTROY_PAPER_BYSELF")) || (job_code.contains("DESTROY_CD_BYSELF"))
					|| (job_code.contains("DESTROY_DEVICE_BYSELF")) || (job_code.contains("SEND"))) {
				String temp_user_iidd = job.getAccept_user_iidd();
				if ((temp_user_iidd != null) && (!temp_user_iidd.equals(""))) {
					setSupervise_user_name(userService.getUserNameByUserId(temp_user_iidd));
				}
			}

			String temp = supervise_user_name;
