 ...
 * 
 */
public class ViewPaperStaticBySeclvAction extends BasicBaseAction {
	...
	public String executeFunction() throws Exception {
		...
		if (query.equalsIgnoreCase("Y")) {
			...
			for (SecLevel seclv : seclvList) {
				seclv_names += seclv.getSeclv_name() + "#";
			}

			int seclvNum = seclvList.size();
			for (int i = 0; i < paperStaticList.size() / seclvNum; i++) {
				int dept_pages = 0;
				for (int j = 0; j < seclvNum; j++) {
					dept_pages += paperStaticList.get(seclvNum * i + j).getPage_count();
				}
