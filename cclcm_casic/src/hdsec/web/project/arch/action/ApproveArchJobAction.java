...
public class ApproveArchJobAction extends ArchBaseAction {
	...
	public void setEventList(List<EventArchBrw> eventList) {
	...
	public String executeFunction() throws Exception {
		if (StringUtils.hasLength(approved)) {// 审批结果
			...
			ApproverUser user = new ApproverUser(getCurUser().getUser_iidd(),
					getCurUser().getUser_name());
			ApproverUser approver = new ApproverUser(next_approver,
					next_approver_name);
			List<EventArchBrw> eventTempList = archService
					.getEventArchListByJobCode(job_code);
			int borrow_status = 0;
			for (EventArchBrw item : eventTempList) {
				borrow_status = item.getBorrow_status();
				continue;
			}
			if (borrow_status == 3) {
				archService.approveRenewJob(job_code, user, approver, approved,
						opinion, "");
				insertCommonLog("审批档案续借任务，审批单编号[" + job_code + "]");
			} else {
				archService.approveJob(job_code, user, approver, approved,
						opinion, "");
				insertCommonLog("审批档案借阅任务，审批单编号[" + job_code + "]");
			}
			return "ok";
		} else {
			...
			try {
				List<ApproverUser> oriList = basicService.getNextApprover(
						job_code, job.getDept_id(), job.getSeclv_code(), job
								.getJobType().getJobTypeCode(), usage_code);
				...
				for (ApproverUser user : userList) {
					List<SecLevel> seclvList = userService
							.getImportSecLevelByUser(user.getUser_iidd());
