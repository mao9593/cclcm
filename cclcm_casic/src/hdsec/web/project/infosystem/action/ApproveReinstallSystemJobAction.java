...
public class ApproveReinstallSystemJobAction extends InfosystemBaseAction {
	...
	private void getHandle2(Integer history_step) {
		...
		for (i = history_step + 1; i <= listSize; i++) {
			if (recordList.get(i).getOpinion().contains("reinstall")) {
				int endIndex = recordList.get(i).getOpinion().lastIndexOf("reinstall");
	...
	public String executeFunction() throws Exception {
		if (history.equals("Y")) {
			...
			if (StringUtils.hasLength(approved)) {// 记录审批结果
				...
				if (event.getEvent_type() == 6 && !software_type.isEmpty()) {// 计算机重装操作系统信息中心处理时将相关信息记录到操作关联表中
