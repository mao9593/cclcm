...
public class AddQuitInternetEventAction extends InfosystemBaseAction {
	...
	public String executeFunction() throws Exception {
		if (StringUtils.hasLength(event_code)) {
			...
			ChangeDeviceEvent event = new ChangeDeviceEvent(event_code, user_iidd, dept_id, apply_time, seclv_code, "",
					"", user_phone, computer_barcode, event_reason, "", 7, summ);// 7:计算机退网
			String jobType_code = "EVENT_QUITINT";
			...
			insertCommonLog("添加计算机退网申请[" + event_code + "]");
			return "ok";
		} else {
			apply_time = new Date();
			apply_time_str = sdf.format(apply_time);
			event_code = getCurUser().getUser_iidd() + "_ADDQUITINTERNET_" + System.currentTimeMillis();
