...
public class AddReinstallSystemEventAction extends InfosystemBaseAction {
	...
	public String executeFunction() throws Exception {
		if (StringUtils.hasLength(event_code)) {
			...
			ChangeDeviceEvent event = new ChangeDeviceEvent(event_code, user_iidd, dept_id, apply_time, seclv_code, "",
					"", user_phone, computer_barcode, event_reason, "", 6, summ);// 6:计算机重装系统申请
			String jobType_code = "EVENT_REINSTALL";
			...
			insertCommonLog("添加计算机重装系统申请[" + event_code + "]");
			return "ok";
		} else {
			apply_time = new Date();
			apply_time_str = sdf.format(apply_time);
			event_code = getCurUser().getUser_iidd() + "_ADDREINSTALLSYSTEM_" + System.currentTimeMillis();
